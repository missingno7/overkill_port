...
wrappers in :mod:`overkill_port.replacements` keep the original ASM addresses
...
# replacements.py; wrappers pass them through LayerSpriteRuntime.
...
class LayerSpriteRuntime:
    ...
    The pure game logic in this module should not import ``replacements.py``.
    ...
    * the fail-fast reporter from replacements.py so unknown paths keep the
...
def predict_layer_sprite_composite_target_768e(cpu, bp: int) -> int | None:
    ...
    Parent scan hooks can use this to compose only dispatch chains whose nested
    compositor is already known.  ``None`` represents the documented immediate
    return path where SS:[BP+0Ch] is FFFFh.
    """
    cs = cpu.s.cs & 0xFFFF
    ss = cpu.s.ss & 0xFFFF
    if cpu.mem.rw(ss, (bp + OBJ_DEST_SLOT_0C) & 0xFFFF) == OFFSCREEN_DESTINATION:
        return None
    dispatch = (cpu.mem.rw(cs, VIDEO_MODE_SELECTOR_OFF) << 3) & 0xFFFF
    dispatch = (dispatch + cpu.mem.rw(ss, (bp + OBJ_ANIMATION_PHASE) & 0xFFFF)) & 0xFFFF
    table = _variant_layer_table(
        LAYER_768E_NORMAL_TABLE,
        LAYER_768E_VARIANT_TABLE,
        cpu.mem.rw(ss, (bp + OBJ_VARIANT_FLAGS) & 0xFFFF),
    )
    return cpu.mem.rw(cs, (table + ((dispatch << 1) & 0xFFFF)) & 0xFFFF)


def is_known_layer_sprite_composite_target(target_ip: int | None) -> bool:
    """Return whether a layer compositor target is verified or narrowly allowed."""
    if target_ip is None:
        return True
    target = target_ip & 0xFFFF
    return target in LIFTED_LAYER_COMPOSITE_TARGETS or target in KNOWN_ORIGINAL_LAYER_COMPOSITE_TARGETS
