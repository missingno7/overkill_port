...
class CPU8086:
    ...
    def execute_opcode(self, op: int, seg_override: str | None, rep: int | None) -> str:
        ...
        if op in (0xD0,0xD1,0xD2,0xD3):
            bits = 8 if op in (0xD0,0xD2) else 16
            count = 1 if op in (0xD0,0xD1) else (s.cx & 0xFF)
    ...
    def string_op(self, op: int, rep: int | None) -> str:
        s = self.s
        width = 2 if op in (0xA5,0xAB,0xAD,0xAF) else 1
        ...
        while count > 0:
            ...
            if op in (0xA4,0xA5):
                val = self.mem.rb(s.ds, s.si) if width == 1 else self.mem.rw(s.ds, s.si)
                ...
                if width == 1: self.set_reg8(0, self.mem.rb(s.ds, s.si))
                else: s.ax = self.mem.rw(s.ds, s.si)
            ...
            if rep is None:
                break
        return ("rep " if rep else "") + {0xA4:"movsb",0xA5:"movsw",0xAA:"stosb",0xAB:"stosw",0xAC:"lodsb",0xAD:"lodsw",0xAE:"scasb",0xAF:"scasw"}[op] + (f" ; {done}" if rep else "")
