...
``scripts/lint.py`` checks imports and layering but not name *resolution*, so a
reference to a name that is never bound -- a forgotten import, a copy/paste that drops
the local binding -- sails through until that branch happens to run.  Two such latent
``NameError`` landmines shipped this way (the dead ``efae`` predictor's undefined
``slot``; ``tandy.postcopy_scaled_blit_375b`` calling an unimported
``_inc_reg16_preserve_cf``), each hidden in a path with no oracle.
