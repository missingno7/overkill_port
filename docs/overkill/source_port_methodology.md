...
Current OVERKILL-specific details are in `docs/overkill/bootstrap_static_boundary.md` and
...
- `docs/overkill/runtime_findings.md` with the actual finding,
- `docs/overkill/island_truth_tables.md` with the island status when relevant,
- `docs/overkill/run_status.md` with the current checkpoint and validation commands,
...
```text
overkill/hooks.py                         exact hook address wrappers only
...
implementation moved from overkill/hooks.py when stable
```
...
Keep overkill/hooks.py as registry glue only.
```
...
4. **`overkill/hooks.py` should gradually die.** It should become registry glue,
...
### Step 1 — Keep Cleaning `overkill/hooks.py`
...
```text
overkill/hooks.py is no longer where logic lives
```
...
and audit commands live in `docs/overkill/runtime_code_staticization.md`.
...
5. overkill/hooks.py should be registry glue only.
