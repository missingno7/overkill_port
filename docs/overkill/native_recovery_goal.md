...
1. **Pick ONE target** from the backlog (top first); keep it finishable + verifiable
   ...
   9 TODO.  The *clean whole-behavior* formulas are recovered:
   ...
   - `b9f0`: target-X wrap -> `b9f0_wrapped_target_x`; reached-target -> `b9f0_reached_target`;
     low-counter helper gate -> `b9f0_low_counter_runs_helper`.
   ...
   STILL EXTRACTABLE (the gates inside the bigger behaviors, a8c7 pattern -- extract the
   pure *condition* even when it guards a side-effectful action; keep the action + flag
   replay adapter-side): `b9f0`'s periodic difficulty-tick helper trigger
   (``[BEDC]==2 ? mask 7Fh : FFh; ([2340] & mask)==mask``) and its `232E==3Fh` spawn gate;
   `aed8`'s timer-expiry + AD5A/ADC9 tail selection.  Mine these before declaring the
   behavior layer done.  The pure *router/dispatch* TODOs (`to_ab77`, `b24d`,
   `dispatch_aa2b`, `b250_selector`, `efae`-dispatch, `ad04`-router, `aa2b`-scan; `8d4f`
   attended-only) carry no liftable condition -- they are correctly adapter-side.
   AFTER the gates = the deep call-tree leaves these behaviors bound-call into (Phase 1's
   ...
   tails.  NOTE: the behaviors deliberately run these via
   `run_original_near_call`/`_run_interpreted_near_call_observed` for byte-for-byte
   internal stack-scratch fidelity, so a leaf lift must reproduce that scratch exactly
   before its shim can collapse to a direct call -- substantial, demo-replay-gated work.
