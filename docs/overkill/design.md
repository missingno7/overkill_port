...
source-port scaffold. Current progress belongs in `docs/overkill/run_status.md`; accumulated
address-level findings belong in `docs/overkill/runtime_findings.md`.
...
`docs/overkill/source_port_methodology.md` and `docs/architecture/package_boundary.md`.  This design document explains the local
...
See `docs/overkill/bootstrap_static_boundary.md` and the importable manifest in
`overkill/bootstrap_boundary.py`.  Use:
...
made that name obvious.  `overkill/hooks.py` and hook wrappers live around layers
2-3; `overkill/<island>/` modules are mostly layers 3-5; future clean
gameplay models will be layers 6-8.

See `docs/overkill/source_port_methodology.md` for the full OVERKILL-specific version of this
pyramid.
...
- `overkill/hooks.py`: thin hook wrappers and staging area for newly lifted code.
...
- `overkill/verification.py`: live differential verifier that runs original ASM and hook
  side by side at a replacement boundary.
- `dos_re/frame_verify.py`: reusable differential frame-verification engine and PNG/diff artifact writer.
- `overkill/frame_verify.py`: OVERKILL adapter that supplies frame boundaries, VRAM slicing, and CGA/EGA/Tandy rendering.
- `overkill/`: game-specific source-port modules for stable
  lifted behavior.
...
`overkill/hooks.py` is the address-facing layer. It should contain the exact
...
unknown original ASM
  -> staged hook in overkill/hooks.py
  -> verified helper in overkill/<island>/
  -> thin wrapper remains in overkill/hooks.py
```
