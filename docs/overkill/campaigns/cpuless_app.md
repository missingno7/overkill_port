...
> **CORRECTION 2026-07-18f -- `254A:04D7` WAS NEVER THE BOOT ROOT.** Several entries below (around
> lines 179, 305-306, 622, 692, 802) describe `254A:04D7` as "the C-startup bootstrap the EXE
> far-jumps to" / the from-EXE cold boot. **That is false.** `254A:04D7` is the ASSET-CONTAINER
> OVERLAY-OPEN routine: `overkill/asset_codecs/container.py` is the VM-free form of exactly that
> address, its IR record is `exits: ['retf'], ints: ['21']`, its first block does INT 21h AX=3D02
> (DOS open), and it returns AX=2 -- a FILE HANDLE. It is far-CALLED from `1010:0248` and
> `1010:C679` and from nowhere else. So every report of "BOOT ROOT RAN TO COMPLETION, byte-identical
> to before" was an overlay open returning a handle: complete on the first run and on every run,
> with **no path onward**. Read the cold-boot-progress claims from that root as reporting nothing.
> The real from-EXE entry is the MZ entry `1C32:000E` (the LZEXE unpacker stub), recorded in
...
autogenerated execution spine. It can, and there is now a cold-start differential that says so with
a number.

**What was actually blocking it** -- four things at once, none of them recovery gaps:

1. The cold probe pointed at a root that could not fail (see the correction above).
2. `plat.boundary_cb` was never set ANYWHERE in the port, so every declared boundary head was inert.
3. The one env-wait that mattered was handled by a host OVERRIDE intercepting `1010:0679` and calling
   a host yield -- flow the host invents, beside the spine rather than inside it -- and that override
   was itself dead code: `getattr(plat, "boundary", None)` found
   `CPUlessPlatformRuntime.boundary(head_cs, head_ip, resume_ip, regs, cost)`, the observer protocol,
   not `OverkillPlatform.boundary(kind)`, the host-yield protocol. Two unrelated protocols sharing a
   method name is not something a None-check can see.
4. The generated `ivec_exec` had no seam for a ROM-BIOS vector, so the game IRQ0 ISR tail-chaining to
   the previous INT 08h handler read as a gap in the game.

**What landed.** `1010:0679` is a declared BOUNDARY HEAD, so the generated body calls
`plat.boundary(0x1010, 0x0679, 0x067F, ...)` itself; `overkill/cpuless_driver.py` answers it; the
override is RETIRED and `OVERRIDES` is empty -- which is the correct state, because with an empty
registry the composite is bit-for-bit the generated program, so a green run proves the CORPUS. Two
dos_re fixes went with it: a boundary head now forces `sp` into the contract, and a chained IRQ
vector into ROM is serviced by the device model through the VMless binding own already-verified
`_chain_iret_stub`.
...
| step | result |
|------|--------|
| declare `1010:0679` a head | promotable **623/626 unchanged**, refusals unchanged, parking 10 -> 20 |
| the 10 functions that lose their CPU-ABI adapter | in NEITHER verification ledger: **no proven coverage lost** |
| declare `1010:50C9` instead | **REGRESSES**: promotable 623 -> 585, contains-call 0 -> 37 (a thunk; the head lands on a transfer) |
| cold run, before the ROM-chain seam | frontier at frame 2, `ivec F000:FF53` |
| cold run, after it | **11999 frames, no missing seam**, every frame cut at `1010:0679` |
| cold-start differential vs the pure oracle | **882 frames byte-identical**, then a real divergence |

**THE CURRENT FRONTIER -- frame 882.** 52 of 32768 B800 bytes differ, spanning rows 59..86 and
x 68..137 across 8 distinct byte columns. The oracle draws `0C`/`FC`/`C0` (bright-red glyph pixels);
...
function, a mid-function RESUME. The CPUless corpus has no resume entries, so the candidate cannot
start where those demos were recorded and their boundary indices are keyed to a start it cannot
reproduce. The attract demo carries ZERO input events, so the 882 proven frames cover the intro
without needing them; the spine demo 24 events need an input path frame-aligned to the `96C8` start.
