...
- The gate has TWO forms.  `clean_window` (FORWARD from one seed) is music-only -- it stays byte-exact
  across a steady window then diverges at the first game event (a 154-write operator burst on L2 at
  gameplay-frame 309), which is how it READS the tempo + proves the music tick.  `per_tick` (the
  STRONGEST form) re-seeds the driver from the true seg-2032 image at EVERY 2032:0063 entry, so it
  verifies the WHOLE demo -- music AND SFX AND page changes: **PASS ~700 ticks each on L1-L6 (all six planet songs: 1/3/7/9/10/8)**,
  zero divergence.  The `Memory.data` bulk slice makes the per-tick 64KB snapshot cheap.
- Registered in the standing-mechanisms header; locked into the suite as
  `tests/test_native_audio_oracle.py` (a 130-frame per-tick window, 6s).  The DGROUP model AND the
  driver tick (music + SFX) are now both proven byte-exact; the remaining charter audio item is only
  the host sound OUTPUT wiring (getting this verified stream to the speakers in play_native).
