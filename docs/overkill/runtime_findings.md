...
- The `1C43:0069` terminator continuation keeps `AX=0000` and `CX=0003` at
  `00FC`; the bootstrap hook now matches that register contract and has a
  snapshot regression pinned to the captured bootstrap evidence.
- `23AD:0069` reaches the same `00FC` continuation after about `547,681`
  interpreted steps from the captured bootstrap snapshot.  Its exit flags keep
  the auxiliary-flag side effect from the final `DEC DX`, so the verifier needs
  a larger ASM budget and the hook now carries that flag through.
- `2032:024F` is more stateful than the first draft suggested: `mov bl,[si+0749]`
  must update live `BX` before the first YM3812 call, and the frequency word is
  pushed/popped around that call after `OR AH,BL`.  The lifted helper now keeps
  the same dead-stack scratch bytes and the first call sees the same register
  file as the original routine.
...
- Added `overkill/sounds/adlib_driver.py` and a
  ...
  The hook is registered at the exact boundary but currently defers to the
  interpreted oracle because the remaining stack/branch shape around the probe
  still needs proof.
- The loaded driver's `2032:0557` helper returns its input with `AL` copied
  from `AH` at the near-return boundary.  That makes the common probe calls
  exit as `0000h` from `0001h` and `6060h` from `6004h`, while preserving the
  `056E` scratch word under the restored stack.
...
`tests/test_overkill/hooks.py`.  After these hooks, `1010:307E` and
...
`overkill/asset_codecs/rle.py`, with the hook wrapper
...
clearly belongs in `overkill/asset_codecs/overlay.py`:
...
`overkill_layer_sprite_draw_768e`.  It reads `SS:[BP+0C]` as the
...
Use the canonical CLI instead of one-off wrapper scripts:

```bash
python -m overkill.cli snapshot assets/OVERKILL --game-root assets --video tandy --sound pc --steps 100000 --trace-tail 128 --out-dir artifacts/evidence/snapshot_after_bootstrap_100k
```
...
`scripts/render_frame.py` (dependency-free, standard-library `zlib` PNG writer)
...
- New `interrupts.py`: `deliver_interrupt(rt, num)` performs the
...
- paces to real time through the modeled game cadence (`--game-hz`);
- renders via `render_frame.render_ppm`, a 256-entry lookup-table decoder that is
...
frame's poll -- the press was lost.  New `keyboard.py`
...
unpaced turbo mode even with explicit `--game-hz 30`.
...
The old `--fps` compatibility knob has been removed.  Real interactive timing is controlled by `--game-hz`; using Tk/SDL repaint cadence as another timer created accidental double-throttling and still did not represent DOS time correctly.
...
`dos_re.cpu.CPU8086` now implements 8086 `DAA` with AL adjustment, AF/CF
...
`dos_re.runtime.create_runtime` now accepts a `command_tail` argument and
...
2. Out-of-aperture shadow storage.  On real EGA,
   ...
   The old one-off page-offset probe script was removed; use `scripts/play.py --video ega --ega-log-starts` plus targeted snapshots/traces for new EGA investigations.
...
- runtime differential verifier (`overkill/verification.py`, `1D1B` → `near_ret`): 300
...
- synthetic oracle `test_ega_spread_masked_composite_wide_13e7_hook_matches_interpreted_asm`
  (rows 1/2/8/16/17, full register+flag+memory equality vs interpreted ASM);
- runtime differential verifier (`overkill/verification.py`, `13E7` → `near_ret`): 300
  consecutive in-game calls compared full-memory against the interpreted routine
  with zero divergence.
...
The negative-`X` escape in the same helper now also unwinds the live `BCF9`
return word after delegating to the shared `AA46` helper.  Before that fix the
standalone lift left the call stack one word too deep on that branch, even
though the currently observed positive-AA71 path remained correct.
...
### 2026-06-13 — overkill/hooks.py staging refactor and 30BA patched row copier

`overkill/hooks.py` has been reduced back toward an address-facing hook wrapper
layer.  Shared 8086-style helper operations now live in
`overkill/asm.py`, while the large gameplay object,
post-move, collision, and object-logic branch family now lives in
`overkill/gameplay/object_runtime.py`.  The wrappers still
register exact `CS:IP` hooks in `overkill/hooks.py`; tests that import legacy
...
`overkill/file_io/overlay_loader.py` with the hook wrapper
...
`docs/dos_re/source_port_methodology.md`.
...
start with exact-address hooks in `overkill/hooks.py`, prove them against the
original ASM, then move stable behavior into a clear module under
`overkill/` while keeping only the address wrapper in
`overkill/hooks.py`.
...
`artifacts/test_oracles/runtime_code_5e42_gameplay_20260613_220042` contains a materialized object
...
- `overkill/runtime_code.py` stores `RuntimeCodeVariant` records and exact
  signatures.
...
### 2026-06-14: object behavior B86D branch lift

`1010:B86D` is a low-level object-family behavior selected by `EFAE` for logic
id `001Dh`, not a renderer or semantic entity.  From
`artifacts/evidence/snapshot_stop_1010_b86d_behavior`, the hot path checks
`DS:A47E`, object X, and `DS:A7A0`, writes `DS:2308=0001`, selects sprite
`0075h`, aligns object current/target coordinates at slot offsets `+02/+04` and
`+32h/+34h`, calls the small original `B729 -> 5DB2` movement setup, then jumps
to the shared `1010:BC4B` postmove/collision boundary.

From `artifacts/evidence/snapshot_stop_1010_b8b0_behavior`, the adjacent hot
`B8B0` branch checks frame counter `DS:2340`, optionally calls the known
`7476` helper on three exact counter values, shifts object X by `-DS:2342`,
selects sprite `0075h` or `0076h`, optionally increments X when `DS:2328==7`,
