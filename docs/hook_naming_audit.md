...
- Tiny address-bound wrappers for the asset/loading codec island were moved out
  of `overkill_port/replacements.py` into
...
- Shared hook-wrapper mechanics now live in
  `overkill_port/games/overkill/hook_wrappers/common.py`.
- The old `overkill_port.replacements` module remains the compatibility import
  surface: importing it still registers all hooks and still re-exports the old
  function objects used by existing tests.
...
| Address | Previous registry label | New registry label | Reason |
| --- | --- | --- | --- |
| `1010:768E` | `overkill_tandy_layer_sprite_draw_768e` | `overkill_layer_sprite_draw_768e` | The routine is a shared layer-sprite draw helper reached outside Tandy-only code paths. The old Python function name remains as an import alias. |
| `1010:75A6` | `overkill_tandy_layer_sprite_draw_75a6` | `overkill_layer_sprite_draw_75a6` | Same shared layer-sprite island; not Tandy-only. The old Python function name remains as an import alias. |
| `1010:7746` | `overkill_tandy_compact_layer_draw_7746` | `overkill_compact_layer_sprite_draw_7746` | Shared compact layer-sprite draw helper; clarified `sprite` and removed misleading Tandy ownership. The old Python function name remains as an import alias. |

Compatibility note: only the registry labels and new primary Python function names
changed. Existing tests/scripts that import the old `overkill_tandy_*` function
names from `overkill_port.replacements` still get aliases to the same handler
objects.
...
- `overkill_file_checksum_loop_c916`, `overkill_packed_read_byte_0624`, and
  ...
  their registry label.  The older unsuffixed names remain compatibility aliases
  only, so tests and old diagnostics can still import them without polluting
  verifier/coverage labels.
- `asset_codecs/startup_graphics.py` was a stale exact copy of
  `rendering/startup_graphics.py` after the previous island move.  It is now a
  small compatibility shim that re-exports the rendering-preparation
  implementation.  This removes the largest exact duplicate without changing
  hook behavior.
