...
Read this whole file before writing code. The reference implementation that
proved this method is the OVERKILL port; **your game is different** — every
concrete address, video mode, and data layout below is an *example* that lives
in the per-game adapter, not in `dos_re`.
...
*End of charter. The reference implementation demonstrating all of the above is
the OVERKILL port; mine it for concrete patterns, but re-derive every address
and format for your own game — and re-verify everything against your own
executable, which is your only oracle.*
