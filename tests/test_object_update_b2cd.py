...
Pins ``b2cd_sprite_offset`` (the level/BDAC/scroll sprite table) and ``object_update_b2cd``'s blocked ->
None path.  Full byte-exact confirmation is the coverage gate vs the VM (1635/1635 on L6_boss, 0 fail).
"""
...
def test_blocked_seek_returns_none():
    # A blocked 5DB2 (table all FFh) -> the reached-waypoint advance loop, not modelled.
    assert object_update_b2cd(
        slot_x=0x50, slot_y=0x50, slot_direction=4, waypoint_x=0x50, waypoint_y=0x50,
