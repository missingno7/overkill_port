"""Driven gate: native _step_yseeker_6b6c6d vs the ORIGINAL F52A/F554/F53F/F55A, over an L6 demo.

Behaviors 0x6B/0x6C/0x6D are planet-0 (mothership) cue spawns -- three thin wrappers over the shared
...
0x6B/0x6C/0x6D, diffs the VM DGROUP at the following BC45 (body done, postmove not yet) against running
native _step_yseeker_6b6c6d over the same pre-state.

Usage:
    python -m overkill.probes.verify_native_behavior_6bcd [demo] [max_frames]
"""
...
from overkill.recovered.adapters.behavior_walk import _step_yseeker_6b6c6d  # noqa: E402
from overkill.recovered.adapters.flat_memory import MutFlatMemory  # noqa: E402

CS = 0x1010
DS = 0x25CC
ENTRIES = {0xF52A: 0x6B, 0xF554: 0x6C, 0xF53F: 0x6D}
BC45 = 0xBC45
PARAMS = {0x6B: (0x50, 0x011F), 0x6C: (0x60, None), 0x6D: (0x70, 0x0124)}
...
def main(argv) -> int:
    ...
    def on_step(cpu):
        ...
        if ip in ENTRIES:
            ...
            y_target, sprite_add = PARAMS[st["beh"]]
            _step_yseeker_6b6c6d(native, st["bp"], y_target, sprite_add)
    ...
    print(f"behaviour 0x6B/0x6C/0x6D steps verified: {st['n']}  diverging: {st['bad']}")
    for line in st["first"]:
        print(f"  DIVERGENCE {line}")
    ok = st["n"] > 0 and st["bad"] == 0
    print("RESULT:", "PASS -- native _step_yseeker_6b6c6d reproduces F55A byte-exact for every "
          f"0x6B/0x6C/0x6D step ({st['n']} steps)" if ok else "FAIL")
