"""Verify the native A114 three-shot burst (``native_a114``) vs the VM.

A114 is an A067 FULL-fanout child (the a96e-schedule / be06==8 weapon): gated by DS:A3A6 == 0 it spawns
THREE A4EA-seed slots (sprite 32h, logic_id 0Ch, substate 7) at per-shot offsets from the schedule entry
[A96E] base {X0=[ptr+2], Y0=[ptr+4]}, threaded so each allocation skips the last.  Step-hook A114 on the
oracle side, project the gameplay pool (DS:2B5C) + cursor (DS:95DA) + the gate (DS:A3A6) + the schedule
(DS:A96E) via a DS-word reader, run ``native_a114``, and at A114's return assert all three spawned slots'
fields + the final cursor equal the prediction.

Usage:
    python -m overkill.probes.verify_native_a114_spawn [demo_name] [max_frames]
"""
...
from overkill.recovered.systems.objects import native_a114  # noqa: E402
...
def main(argv) -> int:
    ...
    def step(self):
        if getattr(self, "_side", "") == "ref" and (self.s.cs & 0xFFFF) == CS:
            ...
            ss = self.s.ss & 0xFFFF
            key = id(self)
            if ip == A114_ENTRY and key not in pending:
                ...
                pred = native_a114(pool, mem.rw(ds, CURSOR_95DA), mem.rw(ds, GATE_A3A6),
                ...
                if p is not None and ip == p[0] and (self.s.sp & 0xFFFF) == p[1]:
                    ...
                    # (pred[-1]).  On a full pool mid-burst the next A4EA RECYCLES (7550, unmodelled) into a
                    # slot that does NOT move the cursor (bx != cursor); native returns only the committed
                    # find_free shots (a partial burst) and does not predict that recycle slot, so cross-check
                    # BX against pred[-1] only when no recycle happened.  DS:95DA is the authoritative check.
    ...
    print(f"demo {demo_name} ({max_frames} frames): native native_a114 three-shot burst vs VM: "
          f"calls={res['calls']} ok={res['ok']} fail={len(res['fail'])}")
    for mism in res["fail"][:5]:
        print(f"  FAIL {mism}")
    ok = res["calls"] > 0 and not res["fail"]
    print("RESULT:", "PASS -- native A114 three-shot burst byte-exact vs the VM across the demo"
          if ok else "CHECK -- no A114 dispatches reached, or a divergence")
