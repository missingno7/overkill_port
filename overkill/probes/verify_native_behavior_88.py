...
Behaviors 0x88 are planet-0 (mothership) cue spawns -- three thin wrappers over the shared
...
0x88, diffs the VM DGROUP at the following BC45 (body done, postmove not yet) against running
native _step_shooter_88 over the same pre-state.

Usage:
    python -m overkill.probes.verify_native_behavior_88 [demo] [max_frames]
"""
...
from overkill.recovered.adapters.behavior_walk import _step_shooter_88  # noqa: E402
...
def main(argv) -> int:
    ...
    print(f"behaviour 0x88 steps verified: {st['n']}  diverging: {st['bad']}")
    for line in st["first"]:
        print(f"  DIVERGENCE {line}")
    ok = st["n"] > 0 and st["bad"] == 0
    print("RESULT:", "PASS -- native _step_shooter_88 reproduces F13C byte-exact for every "
          f"0x88 step ({st['n']} steps)" if ok else "FAIL")
