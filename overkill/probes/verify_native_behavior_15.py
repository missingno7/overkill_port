"""Driven gate: native `_step_controller_15` vs the ORIGINAL 1F8F:03E6, per 0x15 step, over L4.

Behaviour 0x15 is the shared 8D4F waypoint body's DEFAULT (02CB) arrival.  It is NOT in the L1
lockstep (planet-4 only), so it was recovered from the lifter-verified transcription (liftverify
1F8F:03E6 ORACLE_PASSING via a capture_pure_vm_snapshot snapshot).  This gate proves the native handler
byte-exact against the interpreted original: it replays the L4 demo through the pure ref VM, and at
every 8D4F entry whose record carries behaviour 0x15 it captures the pre-state, then at the
far-call return 8D54 (i.e. after 1F8F:03E6 ran) it diffs the VM's DGROUP against running native
`_step_controller_15` over the same pre-state.  8D4F..8D54 is exactly 1F8F:03E6 (the postmove BC4B is
separately gated), so this isolates the recovered handler.

Usage:
    python -m overkill.probes.verify_native_behavior_15 [demo] [max_frames]
"""
...
from overkill.recovered.adapters.behavior_walk import _step_controller_15  # noqa: E402
...
def main(argv) -> int:
    ...
    def on_step(cpu):
        ...
        if ip == STEP_ENTRY:
            bp = s.bp & 0xFFFF
            if cpu.mem.rw(DS, (bp + 0x18) & 0xFFFF) == 0x15:
            ...
            _step_controller_15(native, st["bp"])
