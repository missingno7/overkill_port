"""Driven gate: native `_step_morpher_81` vs the ORIGINAL 1F8F:0452, per 0x81 step, over the L4 demo.

Behaviour 0x81 is the child behaviour 0x7D spawns (8D57 -> 1F8F:0452, jmp BC4B).  Not in the L1
lockstep; recovered from the lifter-verified transcription (liftverify 1F8F:0452 ORACLE_PASSING via a
capture_pure_vm_snapshot snapshot).  This proves the native handler byte-exact: it replays the L4 demo
through the pure ref VM, and at every 8D57 entry whose record carries behaviour 0x81 it diffs the VM's
DGROUP after the far-call return 8D5C (i.e. after 1F8F:0452 ran) against running native
`_step_morpher_81` over the same pre-state (the A954/230A seek scratch is excluded per the walk shadow).

Usage:
    python -m overkill.probes.verify_native_behavior_81 [demo] [max_frames]
"""
...
from overkill.recovered.adapters.behavior_walk import _step_morpher_81  # noqa: E402
...
def main(argv) -> int:
    ...
    def on_step(cpu):
        ...
        if ip == STEP_ENTRY:
            bp = s.bp & 0xFFFF
            if cpu.mem.rw(DS, (bp + 0x18) & 0xFFFF) == 0x81:
    ...
    print(f"behaviour 0x81 steps verified: {st['n']}  diverging: {st['bad']}")
    for line in st["first"]:
        print(f"  DIVERGENCE {line}")
    ok = st["n"] > 0 and st["bad"] == 0
    print("RESULT:", "PASS -- native _step_morpher_81 reproduces 1F8F:0452 byte-exact for every "
          f"0x81 step ({st['n']} steps)" if ok else "FAIL")
