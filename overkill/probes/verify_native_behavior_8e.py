"""Driven gate: native _step_crawler_8e vs the ORIGINAL F52A/F554/F53F/F55A, over an L6 demo.

Behaviors 0x8E are planet-0 (mothership) cue spawns -- three thin wrappers over the shared
...
0x8E, diffs the VM DGROUP at the following BC45 (body done, postmove not yet) against running
native _step_crawler_8e over the same pre-state.

Usage:
    python -m overkill.probes.verify_native_behavior_8e [demo] [max_frames]
"""
...
from overkill.recovered.adapters.behavior_walk import _step_crawler_8e  # noqa: E402
...
def main(argv) -> int:
    ...
    def on_step(cpu):
        ...
        if ip in ENTRIES:
            ...
            _step_crawler_8e(native, st["bp"], level_tiles_from_image(native))
    ...
    print(f"behaviour 0x8E steps verified: {st['n']}  diverging: {st['bad']}")
    for line in st["first"]:
        print(f"  DIVERGENCE {line}")
    ok = st["n"] > 0 and st["bad"] == 0
    print("RESULT:", "PASS -- native _step_crawler_8e reproduces BB48 byte-exact for every "
          f"0x8E step ({st['n']} steps)" if ok else "FAIL")
