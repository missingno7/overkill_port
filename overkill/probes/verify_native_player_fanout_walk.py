"""Verify the native A067 child schedule walks (``native_a3ca`` / ``native_a3ff``) vs the VM.

A3CA (side anchor, DS:A966/A968/A96A/A96C, A3EC 7/1/7/1) and A3FF (mirrored anchor, DS:A962/A964, A3EC
FFFFh, + the A378 follow-up) drive the A41A per-shot variants over the equipped weapon's schedules,
threading the allocator across spawns.  Step-hook each on the oracle side, project the gameplay pool
(DS:2B5C) + cursor (DS:95DA) + the fire state (DS:A958) + the schedule pointers + the gate words, run the
native walk, and at the walk's return assert every spawned slot's fields AND the final cursor equal the
VM's -- the threaded cursor catches a wrong spawn count, the per-slot fields catch a wrong spawn.

Usage:
    python -m overkill.probes.verify_native_player_fanout_walk [demo_name] [max_frames]
"""
from __future__ import annotations

import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[2]
sys.path.insert(0, str(ROOT))

import overkill.frame_verify as fv  # noqa: E402
from dos_re.cpu import CPU8086  # noqa: E402
from dos_re.input_demo import InputDemoPlayback  # noqa: E402
from overkill.frame_verify import FrameVerifyConfig, run_frame_verifier  # noqa: E402
from overkill.gameplay.player_shot_spawn_gap import set_raise_on_encounter  # noqa: E402
from overkill.input_waits import pump_demo_frame  # noqa: E402
from overkill.recovered.domain.object_slots import ObjectPool  # noqa: E402
from overkill.recovered.systems.objects import native_a3ca, native_a3ff  # noqa: E402
from overkill.recovered.views.object_slots import ObjectSlotView  # noqa: E402

CS = 0x1010
A3CA_ENTRY, A3FF_ENTRY = 0xA3CA, 0xA3FF
A958, A3A0, A95E, A3A4, CURSOR_95DA = 0xA958, 0xA3A0, 0xA95E, 0xA3A4, 0x95DA
A3CA_SCHED = (0xA966, 0xA968, 0xA96A, 0xA96C)
A3FF_SCHED = (0xA962, 0xA964)
GAMEPLAY_BASE, GAMEPLAY_COUNT, STRIDE = 0x2B5C, 0x22, 0x38
STRIDE_WORDS = STRIDE >> 1

_FIELDS = (
    "active_word", "scan_enable_or_solid", "direction_or_step", "sprite_or_state",
    "scan_flag", "hazard_class", "logic_id", "substate", "x_word", "y_word",
)


def main(argv) -> int:
    set_raise_on_encounter(False)   # this probe runs the native A378 itself; let the cand replay freely
    demo_name = argv[0] if argv else "demo_play_tandy_L6_different_weapons_20260618_225615"
    max_frames = int(argv[1]) if len(argv) > 1 else 1200
    demo = InputDemoPlayback.load(ROOT / "artifacts" / "demos" / demo_name)
    snapshot = demo.snapshot_path()
    video = str(demo.manifest.get("metadata", {}).get("video", "tandy"))

    res = {"calls": 0, "ok": 0, "shots": 0, "fail": []}
    pending: dict[int, tuple] = {}
    orig_step = CPU8086.step

    def step(self):
        if getattr(self, "_side", "") == "ref" and (self.s.cs & 0xFFFF) == CS:
            ip = self.s.ip & 0xFFFF
            mem = self.mem
            ds = self.s.ds & 0xFFFF
            ss = self.s.ss & 0xFFFF
            key = id(self)
            if ip in (A3CA_ENTRY, A3FF_ENTRY) and key not in pending:
                pool = ObjectPool(base=GAMEPLAY_BASE, stride=STRIDE, slots=tuple(
                    tuple(mem.rw(ds, (GAMEPLAY_BASE + i * STRIDE + 2 * j) & 0xFFFF)
                          for j in range(STRIDE_WORDS))
                    for i in range(GAMEPLAY_COUNT)))
                cursor = mem.rw(ds, CURSOR_95DA)
                state = mem.rw(ds, A958)
                a3a0 = mem.rw(ds, A3A0)

                def read_ds_word(off, _m=mem, _d=ds):
                    return _m.rw(_d, off & 0xFFFF)
                if ip == A3CA_ENTRY:
                    result = native_a3ca(pool, cursor, state,
                                         tuple(mem.rw(ds, p) for p in A3CA_SCHED), a3a0, read_ds_word)
                else:
                    result = native_a3ff(pool, cursor, state,
                                         tuple(mem.rw(ds, p) for p in A3FF_SCHED), a3a0,
                                         mem.rw(ds, A95E), mem.rw(ds, A3A4), read_ds_word)
                ret_addr = mem.rw(ss, self.s.sp & 0xFFFF)
                pending[key] = (ret_addr, (self.s.sp + 2) & 0xFFFF, result, ip)
            else:
                p = pending.get(key)
                if p is not None and ip == p[0] and (self.s.sp & 0xFFFF) == p[1]:
                    _ret, _sp, result, entry = pending.pop(key)
                    mismatches: list = []
                    for shot in result.spawns:
                        slot = ObjectSlotView(mem, ds, shot.slot_offset & 0xFFFF)
                        mismatches += [(f, getattr(slot, f), getattr(shot, f))
                                       for f in _FIELDS if getattr(slot, f) != getattr(shot, f)]
                    cursor = mem.rw(ds, CURSOR_95DA)
                    if cursor != result.final_cursor:
                        mismatches.append(("final_cursor", cursor, result.final_cursor))
                    res["calls"] += 1
                    res["shots"] += len(result.spawns)
                    if not mismatches:
                        res["ok"] += 1
                    else:
                        res["fail"].append((f"{entry:#06x}", mismatches[:4]))
        return orig_step(self)

    CPU8086.step = step
    orig_load = fv._load_runtime
    sides = iter(("ref", "cand"))

    def patched_load(exe, assets, snap, tail):
        rt = orig_load(exe, assets, snap, tail)
        rt.cpu._side = next(sides)
        return rt

    fv._load_runtime = patched_load
    boundary = {"n": 0}

    def pump_inputs(ref_rt, cand_rt):
        boundary["n"], _ = pump_demo_frame(demo, boundary["n"], (ref_rt, cand_rt), ref_rt.cpu)
        boundary["n"] += 1

    cfg = FrameVerifyConfig(video=video, source="candidate", max_frames=max_frames,
                            semantic_state_check=False, stop_on_diff=False, log_every=0)
    try:
        run_frame_verifier(exe=ROOT / "assets" / "OVERKILL", assets=ROOT / "assets",
                           snapshot=str(snapshot), command_tail=b"", config=cfg, pump_inputs=pump_inputs)
    finally:
        fv._load_runtime = orig_load
        CPU8086.step = orig_step

    print(f"demo {demo_name} ({max_frames} frames): native A3CA/A3FF walk vs VM: "
          f"calls={res['calls']} ok={res['ok']} shots={res['shots']} fail={len(res['fail'])}")
    for entry, mism in res["fail"][:5]:
        print(f"  FAIL {entry} {mism}")
    ok = res["calls"] > 0 and not res["fail"]
    print("RESULT:", "PASS -- native A3CA/A3FF schedule walks byte-exact vs the VM across the demo"
          if ok else "CHECK -- no A3CA/A3FF dispatches reached, or a divergence")
    return 0 if ok else 1


if __name__ == "__main__":
    raise SystemExit(main(sys.argv[1:]))
