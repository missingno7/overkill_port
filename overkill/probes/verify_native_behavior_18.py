...
Behaviors 0x18 are planet-0 (mothership) cue spawns -- three thin wrappers over the shared
...
0x18, diffs the VM DGROUP at the following BC45 (body done, postmove not yet) against running
native _step_shooter_88 over the same pre-state.

Usage:
    python -m overkill.probes.verify_native_behavior_18 [demo] [max_frames]
"""
...
from overkill.recovered.adapters.behavior_walk import _step_waypoint_18  # noqa: E402
...
def main(argv) -> int:
    ...
    def on_step(cpu):
        ...
        if (s.cs & 0xFFFF) != CS:
            return
        ip = s.ip & 0xFFFF
        if ip in ENTRIES and st["pre"] is None:   # B98C is also the loop-back target: only the 1st hit
            bp = s.bp & 0xFFFF
            ...
            _step_waypoint_18(native, st["bp"])
    ...
    print(f"behaviour 0x18 steps verified: {st['n']}  diverging: {st['bad']}")
    for line in st["first"]:
        print(f"  DIVERGENCE {line}")
    ok = st["n"] > 0 and st["bad"] == 0
    print("RESULT:", "PASS -- native _step_waypoint_18 reproduces B98C byte-exact for every "
          f"0x18 step ({st['n']} steps)" if ok else "FAIL")
