...
def tandy_b800_next_row(di: int, *, rewind: int = 0) -> int:
    ...
    of the Tandy raster path.  Some present loops first rewind the copied row
    (``-rewind``); pass ``rewind=0`` (the default) for the pure scanline step.
    """
    if rewind:
        di = (di - rewind) & 0xFFFF
    di = (di + 0x2000) & 0xFFFF
