...
function for 1010:5C9A (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
from overkill.cpuless_recovered.func_1010_5c46 import func_1010_5c46

_PARITY = tuple((1 - bin(v).count('1') % 2) == 1 for v in range(256))


def func_1010_5c9a(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, bx=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > 20000000:
            raise RuntimeError('CPUless dispatch spin in 1010:5C9A (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:5C9A
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0x5C9D)
            ...
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0x5CB7)
            _o, _c = func_1010_5c46(mem, plat, _base=_base + _cost + 9, _df=(1 if df else 0), _flags_in=((_flags_in & ~_fmask) | (((0x1 if cf else 0) | (0x4 if pf else 0) | (0x10 if af else 0) | (0x40 if zf else 0) | (0x80 if sf else 0) | (0x800 if of else 0) | (0x400 if df else 0) | (0x200 if intf else 0)) & _fmask)), bp=bp, cx=cx, di=di, ds=ds, dx=dx, si=si, sp=sp, ss=ss)
            ax = _o['ax']
            bp = _o['bp']
            bx = _o['bx']
            cx = _o['cx']
        ...
        if bb == 2:  # 1010:5CC0
            ...
            ax = 0x5
            dx = 0x3CE
            plat.outp(dx, ax, 2, _base + _cost + 13)
            ds = mem.rw(ss, sp)
            sp = (sp + 2) & 0xFFFF
            _cost += 16
