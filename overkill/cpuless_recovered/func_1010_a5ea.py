...
function for 1010:A5EA (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
from overkill.cpuless_recovered.func_1010_a5ed import func_1010_a5ed

_PARITY = tuple((1 - bin(v).count('1') % 2) == 1 for v in range(256))


def func_1010_a5ea(mem, *, bp=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > 20000000:
            raise RuntimeError('CPUless dispatch spin in 1010:A5EA (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:A5EA
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0xA5ED)
            _o, _c = func_1010_a5ed(mem, bp=bp, ss=ss)
            _gm = _c['fmask']
            if _gm:
                _gf = _c['flags']
