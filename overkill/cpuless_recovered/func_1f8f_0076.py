...
function for 1F8F:0076 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
def func_1f8f_0076(mem, *, ax=0, ds=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1F8F:0076 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1F8F:0076
            si = 0x21D8
            cx = 0xAC
            _a = ax
    ...
    return {'ax': ax & 0xFFFF, 'cx': cx & 0xFFFF, 'si': si & 0xFFFF}, {'flags': _flags, 'fmask': _fmask, 'cost': _cost}
