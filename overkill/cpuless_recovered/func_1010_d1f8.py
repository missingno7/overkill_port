...
function for 1010:D1F8 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
from overkill.cpuless_recovered.func_1010_9f1a import func_1010_9f1a
from overkill.cpuless_recovered.func_1010_d213 import func_1010_d213

_PARITY = tuple((1 - bin(v).count('1') % 2) == 1 for v in range(256))


def func_1010_d1f8(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, bx=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > 20000000:
            raise RuntimeError('CPUless dispatch spin in 1010:D1F8 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:D1F8
            _a = mem.rb(ds, 0x98C0)
            _b = 0x0
            _t = _a - _b
        ...
        if bb == 2:  # 1010:D204
            ...
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0xD20E)
            _o, _c = func_1010_d213(mem, bp=bp, ds=ds, sp=sp, ss=ss)
            ax = _o['ax']
            _gm = _c['fmask']
            if _gm:
                _gf = _c['flags']
