...
function for 1010:C9F1 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
def func_1010_c9f1(mem, plat, *, _base=0, ax=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:C9F1 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:C9F1
            dx = 0x3DA
            _a = mem.rb(cs, 0xCA5A)
            _b = 0x1
