...
function for 1F8F:0142 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
from overkill.cpuless_recovered.func_1010_8d8e import func_1010_8d8e

_PARITY = tuple((1 - bin(v).count('1') % 2) == 1 for v in range(256))


def func_1f8f_0142(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, bx=0, cx=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > 20000000:
            raise RuntimeError('CPUless dispatch spin in 1F8F:0142 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1F8F:0142
            di = mem.rw(ds, ((bx) & 0xFFFF))
            ax = (ax & 0xFF00) | ((mem.rb(ds, 0x98C3)) & 0xFF)
            _a = (ax & 0xFF)
