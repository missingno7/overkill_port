...
function for 1010:A81B (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
from overkill.cpuless_recovered.func_1010_4a65 import func_1010_4a65
from overkill.cpuless_recovered.func_1010_7948 import func_1010_7948
...
_LOCAL = {0x5A7E: 0, 0xA81B: 1, 0xA826: 2, 0xA82D: 3, 0xA839: 4, 0xA83F: 5}


def func_1010_a81b(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:A81B (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:5A7E
            dx = bx
            bx = mem.rw(cs, 0x95BC)
            _n = 1
            ...
            if _n:
                ...
                if _n == 1:
                    ...
                    _fmask |= 0x800
                bx = (_t) & 0xFFFF
            _dt = (mem.rw(cs, ((bx + 23180) & 0xFFFF))) & 0xFFFF
            if _dt in _LOCAL:
                _cost += 4
                bb = _LOCAL[_dt]
                continue
        ...
        if bb == 4:  # 1010:A839
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0xA83C)
            _o, _c = func_1010_7948(mem, plat, _base=_base + _cost + 1, _df=(1 if df else 0), _flags_in=((_flags_in & ~_fmask) | (((0x1 if cf else 0) | (0x4 if pf else 0) | (0x10 if af else 0) | (0x40 if zf else 0) | (0x80 if sf else 0) | (0x800 if of else 0) | (0x400 if df else 0) | (0x200 if intf else 0)) & _fmask)), ax=ax, bp=bp, cx=cx, di=di, ds=ds, dx=dx, sp=sp, ss=ss)
            ax = _o['ax']
            bp = _o['bp']
            bx = _o['bx']
            cx = _o['cx']
            ...
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0xA83F)
            _o, _c = func_1010_4a65(mem, _df=(1 if df else 0), cx=cx, di=di, ds=ds, es=es, sp=sp, ss=ss)
            ax = _o['ax']
            bx = _o['bx']
            cx = _o['cx']
            di = _o['di']
