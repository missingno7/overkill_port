...
function for 1F8F:0452 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
def func_1f8f_0452(mem, plat, *, _base=0, _df=0, _flags_in=2, bp=0, bx=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > 20000000:
            raise RuntimeError('CPUless dispatch spin in 1F8F:0452 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1F8F:0452
            mem.ww(ss, ((bp + 8) & 0xFFFF), (0x16A) & 0xFFFF)
            ...
            continue
        if bb == 1:  # 1F8F:046D
            mem.ww(ss, ((bp + 24) & 0xFFFF), (0x93) & 0xFFFF)
            _cost += 1
            bb = 2
            continue
        if bb == 2:  # 1F8F:0472
