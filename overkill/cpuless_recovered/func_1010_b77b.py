...
function for 1010:B77B (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
def func_1010_b77b(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, bx=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > 20000000:
            raise RuntimeError('CPUless dispatch spin in 1010:B77B (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:AC19
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, bp)
            sp = (sp - 2) & 0xFFFF
        ...
        if bb == 2:  # 1010:B786
            ...
            continue
        if bb == 3:  # 1010:B789
            mem.ww(ss, ((bp + 8) & 0xFFFF), (0x77) & 0xFFFF)
            _cost += 2
            bb = 4
            continue
        if bb == 4:  # 1010:BC4B
