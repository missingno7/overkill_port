...
function for 1010:519A (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
_LOCAL = {0x519A: 0, 0x51A6: 1, 0x51B8: 2, 0x51BC: 3, 0x51C0: 4, 0x51D6: 5, 0x51F3: 6}


def func_1010_519a(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, bx=0, cx=0, di=0, ds=0, dx=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > 20000000:
            raise RuntimeError('CPUless dispatch spin in 1010:519A (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:519A
            es = mem.rw(cs, 0x9596)
            _a = mem.rw(ds, 0x21A2)
            _b = 0x0
