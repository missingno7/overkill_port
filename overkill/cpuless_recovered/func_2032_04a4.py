...
function for 2032:04A4 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
def func_2032_04a4(mem, plat, *, _base=0, _df=0, bx=0, ds=0, dx=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 2032:04A4 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 2032:04A4
            si = 0x4B1
            _d = -2 if df else 2
            ax = mem.rw(ds, si)
