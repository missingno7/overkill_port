...
function for 1010:0672 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
def func_1010_0672(mem, *, ss=0):
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:0672 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:0672
            mem.wb(cs, 0x66B, (0x0) & 0xFF)
            _cost += 2
            break
