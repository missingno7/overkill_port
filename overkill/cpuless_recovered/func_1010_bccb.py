...
function for 1010:BCCB (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
from overkill.cpuless_recovered.func_1010_9e69 import func_1010_9e69
from overkill.cpuless_recovered.func_1010_aa46 import func_1010_aa46
from overkill.cpuless_recovered.func_1010_aa71 import func_1010_aa71
...
def func_1010_bccb(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, bx=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:BCCB (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:BCCB
        ...
        if bb == 12:  # 1010:BCF9
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0xBCFC)
            _o, _c = func_1010_aa71(mem, bp=bp, ds=ds, ss=ss)
            ax = _o['ax']
            _gm = _c['fmask']
            if _gm:
                _gf = _c['flags']
        ...
        if bb == 17:  # 1010:BD09
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0xBD0C)
            _o, _c = func_1010_9e69(mem, plat, _base=_base + _cost + 1, _df=(1 if df else 0), _flags_in=((_flags_in & ~_fmask) | (((0x1 if cf else 0) | (0x4 if pf else 0) | (0x10 if af else 0) | (0x40 if zf else 0) | (0x80 if sf else 0) | (0x800 if of else 0) | (0x400 if df else 0) | (0x200 if intf else 0)) & _fmask)), ax=ax, bp=bp, bx=bx, cx=cx, di=di, ds=ds, dx=dx, es=es, si=si, sp=sp, ss=ss)
            ax = _o['ax']
            bp = _o['bp']
            bx = _o['bx']
            cx = _o['cx']
