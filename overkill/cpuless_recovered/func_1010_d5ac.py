...
function for 1010:D5AC (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
from overkill.cpuless_recovered.func_1010_d602 import func_1010_d602
from overkill.cpuless_recovered.func_1010_d612 import func_1010_d612
...
_LOCAL = {0xD5AC: 0, 0xD5B4: 1, 0xD5BB: 2, 0xD5C2: 3, 0xD5EB: 4, 0xD5EF: 5, 0xD5FB: 6}


def func_1010_d5ac(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:D5AC (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:D5AC
            bx = mem.rw(ds, ((di + 2) & 0xFFFF))
            _a = mem.rb(ds, ((di + 8) & 0xFFFF))
            _t = _a - 1
            zf = (_t & 0xFF) == 0
            sf = (_t & 0x80) != 0
            pf = _PARITY[_t & 0xFF]
        ...
        if bb == 1:  # 1010:D5B4
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0xD5B7)
            _o, _c = func_1010_d612(mem, ax=ax, di=di, ds=ds, si=si, ss=ss)
            ax = _o['ax']
            si = _o['si']
            _gm = _c['fmask']
            if _gm:
            ...
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0xD5BA)
            _o, _c = func_1010_d602(mem, ax=ax, di=di, ds=ds, ss=ss)
            ax = _o['ax']
            _gm = _c['fmask']
            if _gm:
                _gf = _c['flags']
