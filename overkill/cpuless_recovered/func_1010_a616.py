...
function for 1010:A616 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
from overkill.cpuless_recovered.func_1010_a648 import func_1010_a648
...
def func_1010_a616(mem, *, bp=0, ds=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:A616 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:A616
            _a = mem.rw(ds, 0x2350)
            _b = 0xB6
            _t = _a - _b
        ...
        if bb == 2:  # 1010:A61F
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0xA622)
            _o, _c = func_1010_a648(mem, bp=bp, ds=ds, ss=ss)
            _gm = _c['fmask']
            if _gm:
                _gf = _c['flags']
