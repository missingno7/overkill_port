...
function for 1010:EFAE (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
def func_1010_efae(mem, plat, *, _base=0, _df=0, _flags_in=2, bp=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:EFAE (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:EFAE
            ax = mem.rw(ss, ((bp + 4) & 0xFFFF))
            mem.ww(ds, 0xD1FE, ax & 0xFFFF)
            ax = mem.rw(ss, ((bp + 2) & 0xFFFF))
            mem.ww(ds, 0xD200, ax & 0xFFFF)
            ...
            if _n:
                ...
                if _n == 1:
                    ...
                    _fmask |= 0x800
                bx = (_t) & 0xFFFF
            _dt = (mem.rw(cs, ((bx + -4156) & 0xFFFF))) & 0xFFFF
            if _dt in _LOCAL:
                _cost += 7
                bb = _LOCAL[_dt]
                continue
