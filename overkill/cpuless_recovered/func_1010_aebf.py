...
function for 1010:AEBF (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
from overkill.cpuless_recovered.func_1010_af63 import func_1010_af63
...
_LOCAL = {0xAEBF: 0, 0xAECA: 1, 0xAECD: 2, 0xAED3: 3, 0xAED6: 4, 0xAEE4: 5, 0xAF60: 6}


def func_1010_aebf(mem, plat, *, _base=0, _df=0, _flags_in=2, bp=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:AEBF (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:AEBF
            ax = 0xB250
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, ax)
