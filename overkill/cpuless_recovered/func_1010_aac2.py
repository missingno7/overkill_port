...
function for 1010:AAC2 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
from overkill.cpuless_recovered.func_1010_aa46 import func_1010_aa46
from overkill.cpuless_recovered.func_1010_aad3 import func_1010_aad3
...
def func_1010_aac2(mem, plat, *, _base=0, _df=0, _flags_in=2, bp=0, bx=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:AAC2 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:AAC2
            ...
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0xAAC8)
            _o, _c = func_1010_aa46(mem, _df=(1 if df else 0), bp=bp, ds=ds, si=si, ss=ss)
            ax = _o['ax']
            si = _o['si']
            _gm = _c['fmask']
            if _gm:
