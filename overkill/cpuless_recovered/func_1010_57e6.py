...
function for 1010:57E6 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
from overkill.cpuless_recovered.func_1010_580b import func_1010_580b
...
_LOCAL = {0x57E6: 0, 0x57ED: 1, 0x57F4: 2, 0x57FB: 3, 0x5800: 4, 0x5BEE: 5}


def func_1010_57e6(mem, plat, *, _base=0, _df=0, _flags_in=2, bp=0, di=0, ds=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:57E6 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:57E6
            _a = mem.rb(ds, 0x98C0)
            _b = 0x0
            _t = _a - _b
        ...
        if bb == 4:  # 1010:5800
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0x5803)
            _o, _c = func_1010_580b(mem, plat, _base=_base + _cost + 1, _df=(1 if df else 0), _flags_in=((_flags_in & ~_fmask) | (((0x1 if cf else 0) | (0x4 if pf else 0) | (0x10 if af else 0) | (0x40 if zf else 0) | (0x80 if sf else 0) | (0x800 if of else 0) | (0x400 if df else 0) | (0x200 if intf else 0)) & _fmask)), bp=bp, di=di, si=si, sp=sp, ss=ss)
            ax = _o['ax']
            bp = _o['bp']
            bx = _o['bx']
            cx = _o['cx']
        ...
        if bb == 5:  # 1010:5BEE
            ...
            if _n:
                ...
                if _n == 1:
                    ...
                    _fmask |= 0x800
                bx = (_t) & 0xFFFF
            _dt = (mem.rw(cs, ((bx + 23550) & 0xFFFF))) & 0xFFFF
            if _dt in _LOCAL:
                _cost += 4
                bb = _LOCAL[_dt]
                continue
