...
function for 1010:477E (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
def func_1010_477e(mem, *, _df=0, di=0, si=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:477E (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:477E
            es = mem.rw(cs, 0x9596)
            ds = mem.rw(cs, 0x9598)
            _d = -2 if df else 2
    ...
    return {'di': di & 0xFFFF, 'ds': ds & 0xFFFF, 'es': es & 0xFFFF, 'si': si & 0xFFFF}, {'flags': _flags, 'fmask': _fmask, 'cost': _cost}
