...
function for 1010:0F0B (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
_LOCAL = {0x0F0B: 0, 0x0F31: 1, 0x0F39: 2, 0x0F49: 3, 0x0F51: 4, 0x0F59: 5, 0x0F61: 6}


def func_1010_0f0b(mem, plat, *, _base=0, _df=0, _flags_in=2, bp=0, dx=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > 20000000:
            raise RuntimeError('CPUless dispatch spin in 1010:0F0B (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:0F0B
            ds = mem.rw(cs, 0x9596)
            es = mem.rw(cs, 0x9596)
            di = 0x99C8
