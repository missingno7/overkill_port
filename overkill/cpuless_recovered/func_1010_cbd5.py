...
function for 1010:CBD5 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
def func_1010_cbd5(mem, *, ax=0, ds=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:CBD5 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:CBD5
            _a = mem.rb(ds, 0x55)
            _b = 0x0
            _t = _a - _b
        ...
        if bb == 1:  # 1010:CBDC
            ...
            break
        if bb == 2:  # 1010:CBDD
            ax = (ax & 0xFF00) | ((mem.rb(ds, 0x54)) & 0xFF)
            _cost += 1
            bb = 3
            continue
        if bb == 3:  # 1010:CBE0
