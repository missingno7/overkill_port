...
function for 1010:587E (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
_LOCAL = {0x5827: 0, 0x587E: 1, 0x5891: 2, 0x5895: 3, 0x5898: 4, 0x58A0: 5, 0x58A4: 6, 0x58DF: 7, 0x58F8: 8}


def func_1010_587e(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:587E (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:5827
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, cx)
            bx = mem.rw(cs, 0x95BC)
        ...
        if bb == 5:  # 1010:58A0
            ax = 0x4
            plat.outp(dx, ax, 2, _base + _cost + 1)
            _cost += 2
            bb = 6
            continue
        if bb == 6:  # 1010:58A4
