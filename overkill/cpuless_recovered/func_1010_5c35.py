...
function for 1010:5C35 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
_LOCAL = {0x5C35: 0, 0x5C74: 1, 0x5C94: 2}


def func_1010_5c35(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, bx=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > 20000000:
            raise RuntimeError('CPUless dispatch spin in 1010:5C35 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:5C35
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0x5C38)
        ...
        if bb == 1:  # 1010:5C74
            ...
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0x5C83)
            _o, _c = func_1010_50c9(mem, plat, _base=_base + _cost + 4, ax=ax, sp=sp, ss=ss)
            ax = _o['ax']
            dx = _o['dx']
            _gm = _c['fmask']
            if _gm:
