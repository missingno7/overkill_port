...
function for 1010:5BDC (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
def func_1010_5bdc(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > 20000000:
            raise RuntimeError('CPUless dispatch spin in 1010:5BDC (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:5BDC
            bx = mem.rw(cs, 0x95BC)
            _n = 1
            _a = bx
            if _n:
                ...
                if _n == 1:
                    ...
                    _fmask |= 0x800
                bx = (_t) & 0xFFFF
            _dt = (mem.rw(cs, ((bx + 23528) & 0xFFFF))) & 0xFFFF
            if _dt in _LOCAL:
                _cost += 3
                bb = _LOCAL[_dt]
                continue
