...
function for 1010:837A (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
_LOCAL = {0x837A: 0, 0x8381: 1, 0x8382: 2, 0x8398: 3, 0x83AB: 4, 0x83B1: 5, 0x83BF: 6, 0x83CA: 7, 0x83D0: 8}


def func_1010_837a(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, bx=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:837A (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:837A
            _a = mem.rw(ds, 0x95FA)
            _b = 0xFFFF
            _t = _a - _b
        ...
        if bb == 7:  # 1010:83CA
            ...
            mem.ww(ss, ((bp + 0) & 0xFFFF), (ax) & 0xFFFF)
            _cost += 3
            break
        if bb == 8:  # 1010:83D0
            mem.ww(ss, ((bp + 8) & 0xFFFF), (0x0) & 0xFFFF)
