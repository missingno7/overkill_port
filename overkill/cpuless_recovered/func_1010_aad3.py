...
function for 1010:AAD3 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
_LOCAL = {0xAAD3: 0, 0xAADA: 1, 0xAADB: 2, 0xAAE2: 3, 0xAAE7: 4}


def func_1010_aad3(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, bx=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:AAD3 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:AAD3
            _a = mem.rw(ds, 0x2384)
            _b = 0x3
            _t = _a - _b
        ...
        if bb == 4:  # 1010:AAE7
            ...
            if _n:
                ...
                if _n == 1:
                    ...
                    _fmask |= 0x800
                bx = (_t) & 0xFFFF
            _dt = (mem.rw(cs, ((bx + -21760) & 0xFFFF))) & 0xFFFF
            if _dt in _LOCAL:
                _cost += 8
                bb = _LOCAL[_dt]
                continue
