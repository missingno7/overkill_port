...
function for 1010:8546 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
_LOCAL = {0x8546: 0, 0x854D: 1, 0x854E: 2, 0x858F: 3, 0x8596: 4, 0x859B: 5}


def func_1010_8546(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, bx=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > 20000000:
            raise RuntimeError('CPUless dispatch spin in 1010:8546 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:8546
            _a = mem.rw(ds, 0x95FA)
            _b = 0xFFFF
            _t = _a - _b
        ...
        if bb == 2:  # 1010:854E
            ...
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, _t)
            _dt = (mem.rw(ds, ((bx + si + 4) & 0xFFFF))) & 0xFFFF
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0x8572)
            ...
            _t = mem.rw(ss, sp)
            sp = (sp + 2) & 0xFFFF
            mem.ww(ds, 0x95FA, (_t) & 0xFFFF)
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0x857F)
            _o, _c = func_1010_837a(mem, plat, _base=_base + _cost + 17, _df=(1 if df else 0), _flags_in=((_flags_in & ~_fmask) | (((0x1 if cf else 0) | (0x4 if pf else 0) | (0x10 if af else 0) | (0x40 if zf else 0) | (0x80 if sf else 0) | (0x800 if of else 0) | (0x400 if df else 0) | (0x200 if intf else 0)) & _fmask)), ax=ax, bp=bp, bx=bx, cx=cx, di=di, ds=ds, dx=dx, es=es, si=si, sp=sp, ss=ss)
            ax = _o['ax']
            bp = _o['bp']
            bx = _o['bx']
            cx = _o['cx']
            ...
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0x8588)
            _o, _c = func_1010_859e(mem, plat, _base=_base + _cost + 20, _df=(1 if df else 0), _flags_in=((_flags_in & ~_fmask) | (((0x1 if cf else 0) | (0x4 if pf else 0) | (0x10 if af else 0) | (0x40 if zf else 0) | (0x80 if sf else 0) | (0x800 if of else 0) | (0x400 if df else 0) | (0x200 if intf else 0)) & _fmask)), ax=ax, bp=bp, di=di, ds=ds, dx=dx, es=es, sp=sp, ss=ss)
            ax = _o['ax']
            bp = _o['bp']
            bx = _o['bx']
            cx = _o['cx']
