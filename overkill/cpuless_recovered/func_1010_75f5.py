...
function for 1010:75F5 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
_LOCAL = {0x75F5: 0, 0x7612: 1, 0x7615: 2}


def func_1010_75f5(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, cx=0, di=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:75F5 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:75F5
            es = mem.rw(cs, 0x9598)
            bx = mem.rw(cs, 0x95BC)
            _n = 1
