...
function for 1010:75A6 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
from overkill.cpuless_recovered.func_1010_75f5 import func_1010_75f5
...
_LOCAL = {0x75A6: 0, 0x75B3: 1, 0x75BB: 2, 0x75D6: 3, 0x75DB: 4, 0x75E3: 5, 0x75E4: 6, 0x7612: 7, 0x7615: 8}


def func_1010_75a6(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, ds=0, dx=0, es=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:75A6 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:75A6
        ...
        if bb == 2:  # 1010:75BB
            ...
            mem.ww(cs, 0x7624, (si) & 0xFFFF)
            mem.ww(cs, 0x7626, (cx) & 0xFFFF)
            di = mem.rw(ss, ((bp + 12) & 0xFFFF))
            _a = di
            _b = 0xFFFF
            _t = _a - _b
            zf = (_t & 0xFFFF) == 0
        ...
        if bb == 3:  # 1010:75D6
            ...
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0x75DA)
            _o, _c = func_1010_75f5(mem, plat, _base=_base + _cost + 2, _df=(1 if df else 0), _flags_in=((_flags_in & ~_fmask) | (((0x1 if cf else 0) | (0x4 if pf else 0) | (0x10 if af else 0) | (0x40 if zf else 0) | (0x80 if sf else 0) | (0x800 if of else 0) | (0x400 if df else 0) | (0x200 if intf else 0)) & _fmask)), ax=ax, bp=bp, cx=cx, di=di, si=si, sp=sp, ss=ss)
            ax = _o['ax']
            bp = _o['bp']
            bx = _o['bx']
            cx = _o['cx']
        ...
        if bb == 6:  # 1010:75E4
            ...
            bx = _t & 0xFFFF
            dx = 0x7658
            _t = (mem.rw(ss, ((bp + 36) & 0xFFFF))) & 0xFFFF
            zf = (_t & 0xFFFF) == 0
            sf = (_t & 0x8000) != 0
            pf = _PARITY[_t & 0xFF]
            cf = of = False
        ...
        if bb == 8:  # 1010:7615
            ...
            bp = 0x10
            cx = bp
            _dt = (mem.rw(cs, ((bx) & 0xFFFF))) & 0xFFFF
            if _dt in _LOCAL:
                _cost += 6
                _fmask |= 0x10 | 0x1 | 0x800 | 0x4 | 0x80 | 0x40
                bb = _LOCAL[_dt]
