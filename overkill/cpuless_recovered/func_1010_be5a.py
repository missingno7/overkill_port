...
function for 1010:BE5A (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
def func_1010_be5a(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, bx=0, cx=0, di=0, ds=0, dx=0, es=0, si=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:BE5A (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:AC19
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, bp)
            sp = (sp - 2) & 0xFFFF
        ...
        if bb == 58:  # 1010:BE6F
            ...
            mem.ww(ss, ((bp + 24) & 0xFFFF), (0x26) & 0xFFFF)
            mem.ww(ss, ((bp + 8) & 0xFFFF), (0x97) & 0xFFFF)
        ...
        if bb == 59:  # 1010:BE84
            ...
            mem.ww(ss, ((bp + 24) & 0xFFFF), (0x26) & 0xFFFF)
            mem.ww(ss, ((bp + 8) & 0xFFFF), (0x91) & 0xFFFF)
            _a = mem.rw(ss, ((bp + 4) & 0xFFFF))
            _b = 0x8
            _t = _a + _b
            zf = (_t & 0xFFFF) == 0
