...
function for 1010:7948 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
_LOCAL = {0x7948: 0, 0x795D: 1, 0x7976: 2}


def func_1010_7948(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, cx=0, di=0, ds=0, dx=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > 20000000:
            raise RuntimeError('CPUless dispatch spin in 1010:7948 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:7948
            es = mem.rw(cs, 0x9592)
            si = mem.rw(ds, 0xA408)
            mem.ww(ds, 0xA406, (0xD) & 0xFFFF)
            mem.ww(ds, 0xA40A, (0x0) & 0xFFFF)
            _cost += 4
            bb = 1
            continue
        if bb == 1:  # 1010:795D
            ...
            if _n:
                ...
                if _n == 1:
                    ...
                    _fmask |= 0x800
                bx = (_t) & 0xFFFF
            _dt = (mem.rw(ds, ((bx + -27170) & 0xFFFF))) & 0xFFFF
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0x796A)
