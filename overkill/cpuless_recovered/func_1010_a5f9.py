...
function for 1010:A5F9 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
from overkill.cpuless_recovered.func_1010_a5fc import func_1010_a5fc
...
def func_1010_a5f9(mem, *, bp=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:A5F9 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:A5F9
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0xA5FC)
            _o, _c = func_1010_a5fc(mem, bp=bp, ss=ss)
            _gm = _c['fmask']
            if _gm:
                _gf = _c['flags']
