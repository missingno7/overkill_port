...
function for 1010:9CB6 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
from overkill.cpuless_recovered.func_1010_4ff9 import func_1010_4ff9
...
def func_1010_9cb6(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, bx=0, cx=0, di=0, ds=0, dx=0, es=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:9CB6 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:9CB6
            sp = (sp - 2) & 0xFFFF
            mem.ww(ss, sp, 0x9CB9)
            _o, _c = func_1010_4ff9(mem, _df=(1 if df else 0), ax=ax, bp=bp, bx=bx, cx=cx, ds=ds, dx=dx, es=es, sp=sp, ss=ss)
            ax = _o['ax']
            bx = _o['bx']
            cx = _o['cx']
            dx = _o['dx']
