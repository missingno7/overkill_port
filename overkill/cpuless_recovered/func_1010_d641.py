...
function for 1010:D641 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
_LOCAL = {0xD5BB: 0, 0xD5C2: 1, 0xD5EB: 2, 0xD5EF: 3, 0xD5FB: 4, 0xD641: 5}


def func_1010_d641(mem, plat, *, _base=0, _df=0, _flags_in=2, ax=0, bp=0, bx=0, cx=0, di=0, ds=0, dx=0, es=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:D641 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:D5BB
        ...
        if bb == 4:  # 1010:D5FB
            ...
            continue
        if bb == 5:  # 1010:D641
            mem.wb(ds, ((di + 11) & 0xFFFF), (0xFF) & 0xFF)
            _cost += 2
            bb = 0
            continue
