...
function for 1010:3824 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
def func_1010_3824(mem, *, _df=0, bp=0, di=0, es=0, si=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:3824 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:3824
            ds = mem.rw(cs, 0x9598)
            _cost += 1
            bb = 1
    ...
    return {'bp': bp & 0xFFFF, 'cx': cx & 0xFFFF, 'di': di & 0xFFFF, 'ds': ds & 0xFFFF, 'si': si & 0xFFFF}, {'flags': _flags, 'fmask': _fmask, 'cost': _cost}
