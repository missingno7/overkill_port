...
function for 1010:D434 (DOS_RE 2.0 stage 2).  DO NOT hand-edit; regenerate.
...
def func_1010_d434(mem, plat, *, _base=0, ax=0, bx=0, di=0, ds=0, dx=0, sp=0, ss=0):
    _cost = 0
    ...
    while True:
        _iters += 1
        if _iters > _ITER_CAP:
            raise RuntimeError('CPUless dispatch spin in 1010:D434 (block %d, cost %d): loop exceeded 20000000 iterations -- an unbounded wait (interrupt-updated flag, or a wrong port after a state divergence)' % (bb, _cost))
        if bb == 0:  # 1010:D434
            _a = mem.rb(ds, 0x98E4)
            _b = 0x1
            _t = _a - _b
        ...
        if bb == 3:  # 1010:D44C
            ...
            _cost += _c['cost']
            sp = (sp + 2) & 0xFFFF
            ax = (ax & 0xFF00) | ((mem.rb(ds, 0xBEDA)) & 0xFF)
